...
nightlife <- scrape_zomato2(category = "drinks-and-nightlife")
dine_out  <- scrape_zomato2(category = "dine-out",
                            total_num = 2255)

# function to process zomato results
process <- function(x) {
  if (!is_tibble(x)) stop ("must supply tibble.")
  
  # set up an empty result in case it's a null response
  result <- tibble::tibble()
  
  # only process if we find one specific correct column header
  if ("name" %in% names(x)){
    result <- x %>%
      select(name, 
             restaurant_address,
      ...
      mutate(lat = pluck(geolocation_data, "lat"),
             lon = pluck(geolocation_data, "long")) %>%
...
scrape_zomato2 <- function(category = c("dine-out", "drinks-and-nightlife"), #, "take-out", "delivery", "cafes"),
                           total_num = 270,
                           batch_size = 50,
                           pause_secs = 15){
  
  category <- match.arg(category, category)
  ...
  for (batch in 1:num_batches){
    ...
    url <- paste0("https://www.zomato.com/ottawa/",category,"?offset=",offset,"&count=",batch_size,"&zpwa=true")
  ...
  nested_content <- tibble::tibble(raw = raw_content) %>%
    mutate(nested = purrr::map(raw, function(x) {
      purrr::pluck(x, "response", "pageData", "restaurants", "basic_info") %>%
        tibble::as_tibble() 
    })) %>%
    mutate(processed = purrr::map(nested, process))
  

  ###########################################################
  ## NOW PUT THE DATA INTO A TIDY FORMAT AND SAVE IT TO CSV
  results <- nested_content %>%
    select(processed) %>%
    unnest(processed)

  results %>%
      write_csv(sprintf("data/restaurants/zomato_%s_%s.csv", category, Sys.Date() ))
 
}

scrape_zomato1 <- function(){
# GOT THESE FROM A SESSION
headers <- c(
  "method"="GET",
  "authority"="www.zomato.com",
...
results %>%
  ...
  write_csv(sprinft("data/restaurants/zomato_restaurants_%s.csv", Sys.Date() ))
# 
