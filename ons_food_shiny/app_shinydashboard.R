...
ui <- shinydashboard::dashboardPage(#title = "ONS Food Data Explorer",#fluidPage(
    
    dashboardHeader(title = "FOOD",
                    tags$li(class = "dropdown",     
                            # selectInput(inputId = "language",
                            #              label = NULL,
                            #              choices = c("French" = "fr", "English" = "en"),
                            #              selected = "en"),
                    
                    
                            tags$li(class = "dropdown", textOutput("logged_user"), style = "padding-top: 15px; padding-bottom: 15px; color: #fff;"),
                            tags$li(class = "dropdown", actionLink("login", label = "EN"))#textOutput("logintext")))
                            )
    ),
    # Application title
   # titlePanel("Food"),
